use bevy::prelude::*;
use bevy_local_commands::{BevyLocalCommandsPlugin, Delay, LocalCommand, ProcessCompleted, ProcessOutput};
...
fn startup(mut commands: Commands) {
    ...
    let id = commands.spawn((cmd, Delay { seconds: 5.0 })).id();
    println!("Will spawn the command in 5s - using entity {id:?}");
}
