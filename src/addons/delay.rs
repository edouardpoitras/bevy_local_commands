...
pub(crate) fn apply_delay(
    mut query: Query<(&mut LocalCommand, &Delay), Without<Process>>,
) {
            ...
            LocalCommandState::Ready => {
                match &*delay {
                    Delay::Fixed(duration) => {
                        local_command.delay = Some(Timer::from_seconds(duration.as_secs_f32(), TimerMode::Once));
                    },
                }
            },
            _ => {},
